 ...
 * last checkin:
 *   $Author: gronemann $
 *   $Date: 2009-12-15 03:19:46 +1100 (Tue, 15 Dec 2009) $
 ***************************************************************/
...
static const size_t MAX_LINE_LENGTH=3000;
...
GmlParser::GmlParser(const char *fileName, bool doCheck) : m_lineNumber(0)
{
	// open file
...
GmlParser::GmlParser(istream &is, bool doCheck) : m_lineNumber(0)
{
	createObjectTree(is,doCheck);
...
void GmlParser::createObjectTree(istream &is, bool doCheck)
{
	...
	// of 254 characters!)
	m_rLineBuffer = new char[MAX_LINE_LENGTH];
...
void GmlParser::initPredefinedKeys()
{
	...
	m_hashTable.fastInsert("image",    imagePredefKey);
	m_hashTable.fastInsert("image_url",imageurlPredefKey);
...
bool GmlParser::getLine()
{
	do {
		...
		m_is->getline(m_lineBuffer,MAX_LINE_LENGTH);
		if (m_is->fail())
			return false;
		m_lineNumber++;
		for(m_pCurrent = m_lineBuffer;
			*m_pCurrent && isspace(*m_pCurrent); ++m_pCurrent) ;
		//cout << "line "<<m_lineNumber<<": "<<m_pCurrent<<endl;
...
bool GmlParser::read(Graph &G, GraphAttributes &AG)
{
	...
	for(; son; son = son->m_pBrother) {
		...
		case nodePredefKey: {
			...
			String imageurl; // imageurl attribute
			String clusterName; // clusterName attribute
			...
			for(; nodeSon; nodeSon = nodeSon->m_pBrother) {
				...
				case imagePredefKey: {
					...
					GmlObject *imageObject = nodeSon->m_pFirstSon;
					for(; imageObject;
						imageObject = imageObject->m_pBrother)
					{
						switch(id(imageObject)) {
						case imageurlPredefKey:
							if(imageObject->m_valueType != gmlStringValue) break;
							imageurl = imageObject->m_stringValue;
							break;
            }
          }
					break;
				...
				case clusterPredefKey: {
					...
					GmlObject *clusterObject = nodeSon->m_pFirstSon;
					for(; clusterObject;
						clusterObject = clusterObject->m_pBrother)
					{
						switch(id(clusterObject)) {
						case clusterPredefKey:
							if(clusterObject->m_valueType != gmlStringValue) break;
							clusterName = clusterObject->m_stringValue;
							break;
            }
          }
					break;
				...
				default:
					//printf("Unrecognised node attribute: %s, ignoring.\n",nodeSon->m_key->key().cstr());
          break;
				}
			}
      ...
      if ( AG.attributes() & GraphAttributes::nodeImageUrl )
      {
        AG.imageUrlNode(m_mapToNode[vId]) = imageurl;
      }
      if (AG.attributes() & GraphAttributes::nodeCluster)
      {
          AG.clusterNode(m_mapToNode[vId]) = clusterName;
      }
