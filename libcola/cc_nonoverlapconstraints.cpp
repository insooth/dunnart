        ...
        vpsc::Variables vs[])
{
    ...
    if (shape1.cluster)
    {
        ...
        COLA_ASSERT(static_cast<size_t>(info.varIndex1 + 1) < vs[0].size());
        right1 = vs[0][info.varIndex1 + 1]->finalPosition;
        COLA_ASSERT(static_cast<size_t>(info.varIndex1 + 1) < vs[1].size());
    ...
    if (shape2.cluster)
    {
        ...
        COLA_ASSERT(static_cast<size_t>(info.varIndex2 + 1) < vs[0].size());
        right2 = vs[0][info.varIndex2 + 1]->finalPosition;
        COLA_ASSERT(static_cast<size_t>(info.varIndex2 + 1) < vs[1].size());
